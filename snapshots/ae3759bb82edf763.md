...
Management Console
Security Control
Home PageAlibaba Cloud Model StudioUser Guide (Models)Get StartedProduct introduction
# What is Alibaba Cloud Model Studio
Updated at: 2026-09-22 01:38:15
Copy as MD
Product
...
## Billing
Activating Model Studio is free. Costs apply only when you invoke models. See Billable items and pricing.
...
- **Call statistics**: After a model call occurs, you can view the monitoring charts at the **minute** level. Log in to the Monitoring page, select **Operations Management** > **Monitoring** in the left navigation pane to enter the monitoring overview page, set query conditions (for example, time range, workspace, etc.), find the target model in the **Models** area and click **View Details** to view call volume, token consumption, success rate, and other statistics. For more information, see Model monitoring.
...
- Go to the Experience Center and select a model.
...
China (Beijing)**,**US (Virginia), Singapore, Japan (Tokyo)**,**Germany (Frankfurt)**,**and China (Hong Kong)
...
- **Turn on**Free quota only**(new users only)**: On the model details page, enable this switch. Service stops when the quota is depleted, preventing paid usage. Applies only to models in the Singapore region within the validity period.
...
Previous：NoneNext: First API call to Qwen
