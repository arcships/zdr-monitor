<!-- https://www.scaleway.com/en/docs/cockpit/concepts/ -->
...
- Refer to the Cockpit FAQ to find out about alerting rules billing
## Cockpit
...
## Contacts
Contacts define who gets notified when an alert is triggered or resolved. Contacts can include email addresses, Slack channels, on-call systems, or phone numbers for text messages. When an alert fires, all configured contacts are notified.
## Custom data
...
## Data exports
Data exports allow you to automatically forward Scaleway metrics and logs from Cockpit to external tools. Only products integrated into Cockpit are eligible, and keep in mind that not all products provide logs. Exports start **from the moment they are created**, meaning data collected beforehand is not included in the export. Exports are configured **per data source** (per region/Project). To export data from multiple regions or Projects, you must create one export per data source. You can also create multiple exports per data source if you want to send data to different export destinations.
## Data sources
...
You can create additional **custom data sources** in the regions where Cockpit is available, to ingest additional observability data in Cockpit.
## Data types
...
Note
- You must enable the alert manager to enable preconfigured alerts.
- Find out which Scaleway products are integrated into Cockpit in the dedicated documentation.
## Metric
...
Important
Refer to the Cockpit FAQ to find out about recording rules billing
## Region
...
You can decide in which region to enable the alert manager and your preconfigured alerts. You can also choose the regions in which to create your data sources, and tokens.
