...
For the configuration reference, including secret types, network access modes, and egress IP ranges, see Secrets & Network. For federating a VM into AWS, GCP, Azure, or a custom verifier without long-lived keys, see OIDC tokens. This page explains the model behind those controls; those pages tell you how to set them.
This is a companion to Cursor's broader security material. See the Trust Center for certifications, subprocessors, and architecture, and the Security and Privacy Hardening reference for the controls you own across all of Cursor. Cursor is SOC 2 Type 2 compliant and commits to at-least-annual penetration testing by reputable third parties.
...
Team admins can go further and lock a Git organization to your Cursor organization with Protected Git Scopes, so only your teams can start Cloud Agents on its repositories. You can also keep sensitive repositories out entirely with a repository blocklist.
...
- **Customer-managed keys.** Enterprise teams can map a customer-managed KMS key (CMEK/BYOK) for Cloud Agent server-side encryption, so you control key rotation and access. See Data encryption.
...
Cloud Agents auto-run terminal commands so they can iterate on tests without stopping for approval on every step. This is more autonomous than the foreground agent, and it changes the risk model: an attacker who plants instructions in content the agent reads (a prompt-injection attack) could try to make the agent exfiltrate code to an external host. See OpenAI's explanation of prompt-injection risk for cloud agents.
The layers that contain this risk:
- **Network egress controls.** Restrict outbound traffic to a default set plus your allowlist, or to your allowlist only, so a compromised agent has nowhere to send data. Enterprise admins can lock the policy org-wide. See Network access.
- **Redacted runtime secrets.** Mark secrets as Runtime Secrets so their values are stripped from the transcript, tool output, and commits and never reach the model.
- **File exclusion.** Add sensitive paths to `.cursorignore` to keep them out of the agent's context.
- **Human-in-the-loop handoff.** Agents open draft pull requests. Nothing merges until a person reviews the change.
- **Signed commits.** Every agent commit is signed with an HSM-backed Ed25519 key and shows a "Verified" badge, so agent-authored changes are attributable and can satisfy signed-commit branch protection. See Signed commits.
For deeper defense, pair these with hooks to enforce policy and log activity at agent lifecycle points, and have Bugbot or Security Agents review agent output before it ships.
## Risk considerations
| Risk | Mitigation |
...
| **Secret exposure** | Encrypted secret storage, redacted runtime secrets kept out of the model, build-only secrets scoped to the Docker build, and OIDC tokens for short-lived cloud federation. |
## Auditability
Cloud Agent activity is logged and attributable.
- **Session logging.** Runs are logged, and team admins can review activity from the Cloud Agents dashboard.
...
- **Audit logs.** Authentication and admin events flow to your audit logs, which Enterprise teams can stream to a SIEM, webhook, or S3.
- **Run diagnostics.** The built-in Cursor Cloud MCP exposes transcripts, run events, environment details, and setup logs for a run.
## Data deletion
| Mechanism | What it removes | How |
| **Archive** | Hides an agent from the dashboard | Archive from the dashboard |
| **Delete Agent API** | An agent's conversation transcript and artifacts | Delete Agent API |
| **Snapshot expiry** | VM snapshots and cached code | Automatic after 90 days of inactivity |
| **Retention policy (Enterprise)** | Conversations older than your chosen window | Retention policies |
| **Account deletion** | The account and its associated data | Delete account |
## FAQ
...
Yes. Cloud Agents can only reach repositories you authorize through your Git provider connection. You control which repositories are available, and admins can lock scopes with Protected Git Scopes or exclude repositories with a blocklist.
Can a Cloud Agent read secrets and credentials?
Configure secrets through the Secrets tab in your dashboard. They're encrypted at rest with KMS, encrypted in transit, and injected as environment variables at runtime. Mark sensitive values as Runtime Secrets to keep them out of the transcript, tool output, and commits. As a matter of practice, keep secrets out of the repository; if sensitive files must live there, add them to `.cursorignore`. For cloud roles, mint OIDC tokens from the VM instead of storing long-lived access keys.
...
How is data deleted?
Archive an agent from the dashboard, or use the Delete Agent API to remove its transcript and artifacts. Full account deletion and Enterprise retention policies remove data on a broader schedule.
## Related pages
- Secrets & Network for secret types, network access modes, egress IP ranges, and signed commits.
- OIDC tokens for short-lived JWTs and cloud federation.
- Privacy and Data Governance for data flows, Privacy Mode, and encryption.
- Security and Privacy Hardening for the controls you configure across Cursor.
- Trust Center for certifications, subprocessors, and architecture.
This summary aids understanding and does not override the MSA, DPA, or other binding contractual terms.
English
- English
- 简体中文
- Русский
