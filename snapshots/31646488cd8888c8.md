<!-- https://docs.fireworks.ai/guides/security_compliance/secure_training -->
> ## Documentation Index
...
| **Bring your own bucket (BYOB)** | *Where the dataset lives* — it stays in your bucket and is read in place, never copied to Fireworks | You need the dataset itself to never leave your storage |
| **Customer-managed keys (CMEK)** | *Who holds the key* for data Fireworks does store — datasets and checkpoints on Fireworks-managed storage | You use managed storage but need to control encryption and revoke access |
CMEK covers artifacts on Fireworks-managed storage, so it complements rather than replaces BYOB. For the strictest governance, combine BYOB with secure RFT, or use the Training API, where no dataset is stored at all.
Inference
...
| Keep data in your own bucket | Use BYOB integration to register an external dataset URL. |
| Control encryption of managed data with your own key | Register a CMEK key from your cloud KMS (AWS, Google Cloud, or Azure); revoke it anytime to cut off Fireworks’ ability to decrypt. |
| Avoid storing a dataset on Fireworks | Use the Training API: no dataset is uploaded or stored; the trainer only receives transient tokenized batches. |
| Revoke Fireworks access post-training | Remove the IAM bindings from your bucket after the job completes. |
For inference-side controls — disabling Response API storage (`store=False`) or deleting a specific response — see the Zero Data Retention policy.
Related Resources
## Zero Data Retention
