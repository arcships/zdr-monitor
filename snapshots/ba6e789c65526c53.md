...
文档备案控制台
官方文档
首页 大模型服务平台百炼 用户指南（模型） 开始使用 选择地域、服务部署范围和接入域名
# 选择地域、服务部署范围和接入域名
更新时间：
复制 MD 格式
产品详情
...
## 选择地域和服务部署范围
不同地域支持不同的服务部署范围，百炼将在所选范围内进行推理调度。例如美国（弗吉尼亚）支持全球和美国，德国（法兰克福）支持全球和欧盟等。完整的地域与服务部署范围对应关系，请前往控制台查看。
## 选择接入域名
...
存量业务兼容，建议迁移至业务空间专属域名。DashScope 域名（dashscope.aliyuncs.com）自2026年9月30日起不再支持新特性。
快速体验、功能验证，不建议用于生产环境。
| **鉴权方式** |
仅访问当前业务空间
可访问所有业务空间
...
API Key
可用模型
- **德国（法兰克福）、日本（东京）、中国香港、美国（弗吉尼亚）**地域通过**业务空间（Workspace）**区分服务部署范围，开始调用前需前往业务空间管理页面创建业务空间并选择服务部署范围：德国（法兰克福）（全球/欧盟）、日本（东京）（全球/日本）、中国香港（全球/中国香港）、美国（弗吉尼亚）（全球/美国）。
- **华北2（北京）**和**新加坡地域**各仅支持一种服务部署范围，无需选择。
**说明**美国（弗吉尼亚）地域的部署模式已升级。如果正在使用带 `-us` 后缀的模型，请尽快完成切换升级，详情请参见升级公告。
## 迁移至业务空间专属域名
...
- 模型调用计费 — 各地域价格
- 限流— RPM、TPM 限制
- 获取与配置 API Key — 创建和管理 Key
- Base URL总览 — 模型服务调用地址
- 概述
上一篇：Base URL总览下一篇：产品计费
该文章对您有帮助吗？
反馈
