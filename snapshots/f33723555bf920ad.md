...
1. A user executes a notebook cell, which results in an error.
2. Databricks attaches metadata to a request and sends it to a Databricks-hosted large-language model (LLM). All data is encrypted at rest. Customers can use a customer-managed key (CMK).
3. The Databricks-hosted model responds with the suggested code edits to fix the error, which is displayed to the user.
On this page
- Data sent to the models
- What services and models do partner-powered AI assistive features use?
- What data is sent to the models?
