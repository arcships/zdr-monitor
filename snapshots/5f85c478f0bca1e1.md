...
_This article covers Codex, OpenAI's coding agent. If you're interested in ChatGPT Work, a separate ChatGPT experience for longer research and deliverable tasks, see__ChatGPT Work and Codex__; for technical Codex details, see the__Codex page__on our developers site. Codex is included across ChatGPT plans._
...
_Learn more about__ChatGPT Voice__—including ordinary Chat Voice and Voice in Work and Codex on desktop._
# What is Codex?
...
For more information, including higher-usage options for individuals and Codex credit plans for business users, visit**chatgpt.com/pricing**.
...
If you’re asked to add or verify a phone number when accessing Codex in the ChatGPT desktop app, see What does phone verification look like?
...
Users with permission to administer Codex can disable full CDP access from Managed configuration by setting `browser_use_full_cdp_access` to `false`. Disabling Browser use also disables CDP.
For setup and approval details, see Developer mode. Note admins/owners will first need to enable thein-app browser before enabling developer mode.
...
Codex usage on token-based Enterprise agreements is billed in US dollars (USD), instead of deducted from credits. Ask your admin which billing model applies. For applicable rates, see: Enterprise token-based rate card. For billing concepts, see: Token-based billing for ChatGPT Enterprise.
...
Learn more about Sharing feedback, evaluation and fine-tuning data, and API inputs and outputs with OpenAI.
### Pro and Plus
...
ChatGPT Voice used inside Work or Codex on desktop is metered separately, while tasks started through Voice use the same shared agentic pool as Work and Codex on plans that use shared allowances or credits.Learn more.
...
For more details, see Codex usage limits.
...
* ChatGPT Business release notes Review the latest features and updates for the ChatGPT Business plan.
