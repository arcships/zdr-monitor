...
- BGE Large (En)
For Anthropic Claude Sonnet 5, if your workspace is not in a US region but is in a supported Model Serving region, you can enable cross-Geo data processing to access this model.
EU and US models
The following models are available in pay-per-token EU and US supported regions.
- OpenAI GPT-6 Sol
- OpenAI GPT-6 Luna
- OpenAI GPT-6 Astra
- OpenAI GPT-5.6 Sol
