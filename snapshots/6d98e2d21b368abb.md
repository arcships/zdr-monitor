<!-- https://docs.factory.ai/enterprise/privacy-and-data-flows.md -->
# Data Flows & Privacy
...
- Droid exports **OTLP metrics** by default to the endpoint pinned in org-managed settings (`telemetry.endpoint`) or configured per machine with `OTEL_TELEMETRY_ENDPOINT`. You can also opt in to message content trace spans, which are sent only to your collector.
...
- Customer metrics export runs as a fan-out alongside Factory's own metrics export in connected deployments. In airgapped deployments the fan-out has one leg: your collector, with no Factory-bound export built at all. Airgapped deployments do not use Factory-hosted analytics.
- Organizations that cannot collect per-individual analytics can pin `telemetry.granularity: aggregate`, which strips user identifiers from every datapoint at the source on both legs.
...
For telemetry setup and the hosted Analytics API, see Telemetry & Analytics; for the exported metric names and schema, see the Telemetry Data Reference; for how this data maps to audit and compliance workflows, see Compliance & Audit.
...
In fully airgapped environments, Factory never receives any runtime data; you are responsible for all retention and residency decisions. See Airgapped Deployment for the runtime behavior and network contract.
