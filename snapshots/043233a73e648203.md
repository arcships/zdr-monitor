<!-- https://docs.digitalocean.com/products/inference/how-to/use-prompt-caching/index.html.md -->
...
Use prompt caching with the serverless inference chat completion and responses APIs to cache context and use it in future requests. If part of your request is already cached, you are charged a lower price for those cached tokens, and the standard price for the remaining input tokens. This significantly reduces the cost for inference. Open-source models cache context automatically.
## Automatic Prompt Caching for Anthropic Models
When you send eligible requests through an Inference Router, we automatically apply the `cache_control` parameter. No manual configuration or opt-in is required to enable caching.
**Note**:
...
For a detailed example, see Use Model Affinity for Caching.
