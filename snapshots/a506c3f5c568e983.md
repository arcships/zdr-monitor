<!-- https://docs.digitalocean.com/products/inference/how-to/create-manage-agent-knowledge-bases/index.html.md -->
...
If you want to attach the knowledge base to agents, choose the same region as your agents to reduce latency. Most Agent Platform infrastructure is in TOR1, so we recommend the default region.
...
To check and enable reranking, go to the knowledge base’s **Settings** tab.
Once reranking is enabled, reranking tokens are billed separately from vectorizing retrieval queries. For more information, see knowledge base pricing.
...
For supported formats, upload recommendations, and source-specific limits, see Limits.
To manage your data sources, go to the Control Panel, in the left-hand menu, under **INFERENCE**, click **Agent Platform**, and then the **Knowledge Bases** tab.
...
### cURL
Send a POST request to `https://api.digitalocean.com/v2/gen-ai/indexing_jobs`.
Using cURL:
```shell
curl -X POST \
-H "Content-Type: application/json" \
...
To edit a knowledge base, go to the Control Panel, in the left menu, under **INFERENCE**, click the **Agent Platform** tab, and then the **Knowledge bases** tab.
