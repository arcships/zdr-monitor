...
For AI agents: documentation index at /llms.txt — fetch to discover all Snowflake documentation pages. Markdown version of this page: /en/user-guide/snowflake-cortex/cortex-agents-threads.md
...
1. Create a new thread using Create thread.
...
Note
You must pass an assistant message ID as the `parent_message_id` to ensure the LLM functions as expected. You cannot pass a user message ID. If you have lost track of the last message ID, use Describe thread to list all messages in the thread.
Copy code
"thread_id": 1234,
...
Copy code
On this page
1. Start a new thread and use it with Agent Run
2. Read the returned message IDs
3. Continue the conversation
4. Retrieve the latest summary and subsequent messages
5. Troubleshooting
