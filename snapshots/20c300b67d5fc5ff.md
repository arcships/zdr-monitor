...
Log inDashboard
# Unified Billing
Last updated Sep 17, 2026|Copy as Markdown|View as Markdown|Agent setup
...
2. **BYOK (stored key)** — if no provider key is on the request and the gateway has a stored key for the provider under the `default` alias, that key is used.
...
The `cf-aig-byok-alias` header selects a non-default alias only on direct provider-passthrough requests.
...
1. Log in to the Cloudflare dashboard ↗ and go to **AI** > **AI Gateway**.
Go to **AI Gateway** ↗
2. Select your gateway.
3. Go to **Settings** and turn on **Require provider credentials**.
4. Confirm the change.
Send a `PUT` request to update the gateway. Include `byok_only: true` in the request body.
...
Refer to the binding reference for the full API surface.
### HTTP API
...
"messages": [{"role": "user", "content": "What is Cloudflare?"}]
Refer to REST API for more details on all available endpoints.
...
- Groq
### Spend limits
Set spend limit rules on individual gateways to cap spend, scoped by model, provider, or custom metadata dimensions like user or team.
...
3. Send a `PUT` request to update the gateway and include `zdr: true` or `zdr: false` in the request body.
