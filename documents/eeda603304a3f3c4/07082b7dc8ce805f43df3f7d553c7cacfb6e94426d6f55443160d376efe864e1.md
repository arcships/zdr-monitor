Title: Models - Docs - Kiro

URL Source: https://kiro.dev/docs/models
...
**Claude Fable 5.1** is rolling out gradually to Kiro Enterprise organizations with model governance enabled. It has a 1M context window and a 6x credit multiplier. Inference runs only in US East (N. Virginia) (`us-east-1`). Organizations with access can add Fable 5.1 from **Manage approved list** and make it available to their users. See [Claude Fable 5.1](https://kiro.dev/docs/models/available-models/#claude-fable-51-preview) for capabilities and data-handling details.

US and EU are geographies, each spanning several AWS Regions rather than a single Region. All authentication methods are supported for every model in the comparison table. For the Regions in each geography and the endpoint that serves your requests, see [Inference endpoint regions](https://kiro.dev/docs/models#inference-endpoint-regions).
...
Cross-region inference does not change where your data is stored. Models marked experimental are an exception to the table above: they may be processed in commercial AWS Regions worldwide, including outside your profile's geography. See [data protection](https://kiro.dev/docs/privacy-and-security/data-protection/#supported-regions-for-kiro-cross-region-inference) for the full reference, and [Amazon Bedrock cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-profiles-support.html) for how inference profiles route requests.
...
Model availability can vary by country or region. Kiro's model offerings align with each provider's usage and geographic requirements. For more information, see supported countries and regions: [OpenAI](https://platform.openai.com/docs/supported-countries), [Anthropic](https://www.anthropic.com/supported-countries), [MiniMax](https://www.minimax.io/), [Zhipu AI](https://z.ai/) (GLM), [DeepSeek](https://www.deepseek.com/), and [Qwen](https://qwenlm.github.io/).
...
*   **Factor model cost into your tier:** If you primarily use Opus, consider Pro+, Pro Max, or Power for more credits. See [plans and billing](https://kiro.dev/docs/billing/) for details.

For detailed descriptions of each model's capabilities, strengths, and lifecycle status, see [Available models](https://kiro.dev/docs/models/available-models/).
