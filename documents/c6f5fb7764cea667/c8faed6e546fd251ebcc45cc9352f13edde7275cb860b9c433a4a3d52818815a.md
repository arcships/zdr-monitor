Title: Data residency on Modal

URL Source: https://modal.com/docs/guide/data-residency
...
## Region selection[](https://modal.com/docs/guide/data-residency#region-selection)
...
For a full list of available regions, please visit our [region selection documentation](https://modal.com/docs/guide/region-selection) page.

‡ _Functions and Sandboxes set the container region with `region=`; Servers use `compute\_region=`, and Endpoints use `--compute-region`._

| **Service** | **Selectable container region?** | **Selectable routing region?** |
| --- | --- | --- |
| [Endpoints (Dedicated)](https://modal.com/docs/guide/dedicated-endpoints) | [Yes](https://modal.com/docs/guide/dedicated-endpoints#configure-capacity-and-placement) | [Yes](https://modal.com/docs/guide/dedicated-endpoints#configure-capacity-and-placement) |
| [Endpoints (Shared)](https://modal.com/docs/guide/shared-endpoints)* | No | No |
| [Functions](https://modal.com/docs/guide/functions) | [Yes](https://modal.com/docs/guide/region-selection#specifying-a-container-region) | [Yes](https://modal.com/docs/guide/region-selection#regional-routing) |
| [Sandboxes](https://modal.com/docs/guide/sandboxes) | [Yes](https://modal.com/docs/guide/region-selection#specifying-a-container-region) | N/A† |
| [Servers](https://modal.com/docs/guide/servers) | [Yes](https://modal.com/docs/guide/servers#request-routing) | [Yes](https://modal.com/docs/guide/servers#request-routing) |

* _Shared Endpoints route through `us-west`, and containers are distributed across Modal’s global compute pool._

† _Sandboxes don’t use a routing region: data-plane requests (including encrypted tunnels, Sandbox Connect Tokens, `exec` input/output) go directly to the container in your selected region. Sandbox lifecycle requests (e.g. create and terminate) are routed through our control plane in `us-east`._

### Region pinning is strict[](https://modal.com/docs/guide/data-residency#region-pinning-is-strict)
...
## Where data is stored[](https://modal.com/docs/guide/data-residency#where-data-is-stored)

In this section, we’ll outline where logs and other data may be stored for different Modal products. For relevant retention policies, please refer to the data retention table in our [security and privacy documentation](https://modal.com/docs/guide/security#data-retention).

Dedicated Endpoints, Shared Endpoints, and Servers do not [store request and response payloads](https://modal.com/docs/guide/security#modal-inference-endpoints).
...
### Data at rest in durable storage products[](https://modal.com/docs/guide/data-residency#data-at-rest-in-durable-storage-products)

Durable storage for Images, Volumes, Dicts, Queues, and Secrets resides in object storage located in the United States.

For durable storage, such as Volumes, you can opt to bring your own storage via [Cloud bucket mounts](https://modal.com/docs/guide/cloud-bucket-mounts), which support various cloud provider buckets in the region of your choice.

## Compliance[](https://modal.com/docs/guide/data-residency#compliance)
...
Per our [shared responsibility model](https://modal.com/docs/guide/security#shared-responsibility-model), it is your responsibility to set the configuration to meet your obligations, such as General Data Protection Regulation (GDPR) compliance.

For more information, please see our Security Portal at [trust.modal.com](https://trust.modal.com/).

### Meeting your regional compliance requirements[](https://modal.com/docs/guide/data-residency#meeting-your-regional-compliance-requirements)
...
### Controlling where your workloads run[](https://modal.com/docs/guide/data-residency#controlling-where-your-workloads-run)

*   **Pin container and routing regions when you have strict residency requirements.** Select container and routing regions for supported products at creation time. Region selection is strict: workloads pinned to a region will only run there. For the capacity and pricing tradeoffs, see [Region pinning is strict](https://modal.com/docs/guide/data-residency#region-pinning-is-strict) above.

### Controlling what customer data leaves your region[](https://modal.com/docs/guide/data-residency#controlling-what-customer-data-leaves-your-region)

*   **Bring your own storage.** You can mount your own object storage, such as an Amazon S3 bucket in your region, into your containers with a [Cloud bucket mount](https://modal.com/docs/guide/cloud-bucket-mounts). Data at rest then stays in your region, under your own retention and access policies.
...
*   **Use regional proxies if your use case requires static egress IPs.** If you use [Modal Proxies](https://modal.com/docs/guide/proxy-ips) for static egress IPs, that traffic routes through the proxy region regardless of your container region. Data is encrypted in transit.
...
### Controlling what new customer data gets created[](https://modal.com/docs/guide/data-residency#controlling-what-new-customer-data-gets-created)
