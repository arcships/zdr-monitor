...
**Zero Data Retention Products.** Baseten adheres to a Zero Data Retention ("**ZDR**") posture across all inference products. Unless a customer, acting through an authorized representative, requests otherwise in writing, ZDR is the default configuration for the customer's use of the inference products; Baseten does not log Customer Content contained in inference inputs or outputs for human review; and Baseten will not retain Customer Content contained in inference inputs or outputs at rest without the customer's prior written consent. Any consented retention will be limited to the minimum necessary to provide the requested functionality, restricted to personnel with a need to know, and deleted when no longer required. Product-specific details are set forth below.
