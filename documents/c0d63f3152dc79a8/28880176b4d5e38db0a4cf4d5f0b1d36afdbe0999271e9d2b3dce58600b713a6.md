...
URL Source: https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth

Published Time: Mon, 21 Sep 2026 07:47:37 GMT

Markdown Content:
Qwen Code’s first-run `/auth` menu has three top-level options. Pick the one that matches how you want to run the CLI:

*   **Alibaba ModelStudio**: official recommended setup. Opens a sub-menu with **Coding Plan** (for individual developers · weekly quota included), **Token Plan** (for teams and companies · usage-based billing with a dedicated endpoint), or **Standard API Key** (connect with an existing ModelStudio API key). Token Plan and Standard API Key also enable the built-in [`web_search` tool](https://qwenlm.github.io/qwen-code-docs/en/developers/tools/web-search/) with no extra setup.
...
## Option 1: Qwen OAuth (Discontinued)[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#option-1-qwen-oauth-discontinued)
...
In non-interactive or headless environments (e.g., CI, SSH, containers), you typically **cannot** complete the OAuth browser login flow. In these cases, please use the Alibaba Cloud Coding Plan or API Key authentication method.

## 💳 Option 2: Alibaba Cloud Coding Plan[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#-option-2-alibaba-cloud-coding-plan)
...
### Interactive setup[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#interactive-setup)
...
### Headless or scripted setup[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#headless-or-scripted-setup)
...
### Alternative: configure via `settings.json`[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#alternative-configure-via-settingsjson)
...
## 🪙 Option 3: Alibaba Cloud Token Plan[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#-option-3-alibaba-cloud-token-plan)
...
### Interactive setup[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#interactive-setup-1)
...
### Headless or scripted setup[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#headless-or-scripted-setup-1)
...
### Alternative: configure via `settings.json`[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#alternative-configure-via-settingsjson-1)
...
## 🚀 Option 4: API Key (flexible)[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#-option-4-api-key-flexible)
...
### Recommended: One-file setup via `settings.json`[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#recommended-one-file-setup-via-settingsjson)
...
The sections below explain each part in more detail. If the quick example above works for you, feel free to skip ahead to [Security notes](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#security-notes).
...
#### Supported protocols[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#supported-protocols)
...
#### Step 1: Configure models and providers in `~/.qwen/settings.json`[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#step-1-configure-models-and-providers-in-qwensettingsjson)
...
Don’t forget to also set `env`, `security.auth.selectedType`, and `model.name` alongside `modelProviders` — see the [complete example above](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#recommended-one-file-setup-via-settingsjson) for reference.
...
When using the `env` field in `settings.json`, credentials are stored in plain text. For better security, prefer `.env` files or shell `export` — see [Step 2](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#step-2-set-environment-variables).

For the full `modelProviders` schema and advanced options like `generationConfig`, `customHeaders`, and `extra_body`, see [Model Providers Reference](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/).

#### Step 2: Set environment variables[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#step-2-set-environment-variables)
...
This is the approach used in the [one-file setup example](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#recommended-one-file-setup-via-settingsjson) above. It’s convenient for keeping everything in one place, but be mindful that `settings.json` may be shared or synced — prefer `.env` files for sensitive secrets.
...
#### Step 3: Switch models with `/model`[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#step-3-switch-models-with-model)
...
## Removed `qwen auth` CLI command[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#removed-qwen-auth-cli-command)
...
## Security notes[](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth#security-notes)
