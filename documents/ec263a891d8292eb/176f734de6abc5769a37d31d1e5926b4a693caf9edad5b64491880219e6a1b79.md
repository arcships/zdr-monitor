Title: SpaceXAI — Creators of Grok, the AI Chatbot

URL Source: https://x.ai/legal/faq

Markdown Content:
## [**SpaceXAI Consumer FAQs**](https://x.ai/legal/faq#spacexai-consumer-faqs)

_May 12, 2025_

**This page is designed to address questions from individual users of Grok who use the Grok mobile app ([iOS](https://apps.apple.com/app/grok/id6670324846) or [Android](https://play.google.com/store/apps/details?id=ai.x.grok)) or the [Grok.com](http://grok.com/) website.**

If you are a SpaceXAI business or enterprise customer, please refer to our [Enterprise FAQ](https://x.ai/legal/enterprise/faq). If you use Grok on the X platform, please see the [X help center.](https://help.x.com/en/using-x/about-grok)

**Note: SpaceXAI does not sell or issue tokens, coins, or crypto.**

## [**Who is SpaceXAI?**](https://x.ai/legal/faq#who-is-spacexai)

SpaceXAI LLC ("**SpaceXAI**") is a United States-based company working on building artificial intelligence tools to accelerate human scientific discovery. We are guided by our mission to advance our collective understanding of the universe. SpaceXAI is a separate company from X Corp. ("**X**", previously Twitter).

## [**What is Grok?**](https://x.ai/legal/faq#what-is-grok)

Grok is a conversational generative AI powered by SpaceXAI's state-of-the-art Large Language Models. Grok performs a variety of tasks, including natural language processing, question answering (voice and text), information retrieval, creative writing, image generation, image editing, image and video understanding, and coding assistance.

You can make Grok your unique and entertaining companion. Grok is designed to answer almost any question with a touch of wit and humor, while providing helpful and insightful responses. You control how Grok responds to you by your choice of features (ex., choosing different personas) and by your choice of words and tones. For instance, you could instruct Grok to interact in a “fun” manner, so that Grok responds with jokes and humor like your funny uncle who juggles at the dinner table. As another example, you could instruct Grok to be “unhinged” which may result in Grok responding like an amateur stand-up comic who is still learning the craft – sometimes being objectionable, inappropriate, and offensive. As yet another example, you could instruct Grok to role-play as a wizard, and Grok would use wizardly language to describe magical situations. You direct the interaction with Grok. We hope you enjoy using Grok to stay informed and entertained.

## [**Where can I use Grok?**](https://x.ai/legal/faq#where-can-i-use-grok)

Grok is available as a standalone conversational AI chatbot on your mobile phone ([iOS](https://apps.apple.com/app/grok/id6670324846)) and ([Android](https://play.google.com/store/apps/details?id=ai.x.grok)), and on your browser ([grok.com](https://grok.com/)). Depending on your location, limited free access and paid subscription plans with full features of Grok unlocked are available.

Grok is also available through the X platform. If you use Grok on X, please see the [X help center](https://help.x.com/en/using-x/about-grok) for your questions. Your access on X is subject to the [X Terms of Service](https://x.com/en/tos) and [X Privacy Policy](https://x.com/en/privacy).

We also offer Grok services to business and enterprise customers, such as our Enterprise API. You can learn more about these services [here](https://x.ai/enterprise-api).

## [**Can children or minors use Grok?**](https://x.ai/legal/faq#can-children-or-minors-use-grok)

Grok is not directed to children or minors under the age of 13. You must confirm that you are at least 13 years old to use Grok. If you are a teenager between the ages of 13 and 17 years old, you must have your parent or legal guardian’s permission to use Grok, and they must agree to our Terms of Service. While we have taken measures to limit undesirable training data and outputs, depending on the features that you choose to use, the Service could produce output that is not appropriate for all ages. For instance, if users choose certain features or choose to input suggestive or coarse language, Grok may respond with some dialogue that may involve coarse language, crude humor, sexual situations, or violence. We urge parents to exercise care in monitoring the use of Grok by their teenagers. Moreover, parents or guardians who choose to use certain features of Grok to aid in their interactions with their children, including regarding educational, enlightening, or entertaining discussions they have with their children, must make use of the relevant data controls in the Settings provided in the Grok apps to select the appropriate features and limitations for their needs.

## [**How does Grok work?**](https://x.ai/legal/faq#how-does-grok-work)

Grok works by using advanced algorithms to understand and respond to your prompts. When you submit a prompt or search query, Grok interprets your words and other inputs based on natural language processing and patterns from its training data.

Grok has a unique feature that allows it to search public X posts and perform real-time web searches, allowing it to respond with up-to-date information and insights on a wide range of topics. You can also ask Grok to respond in the conversational style of your choosing. Plus, you can help improve Grok by rating its responses and submitting the reason why you believe a Grok response needs addressing.

You can interact with Grok by typing or, on iOS devices (and Android devices soon), speaking with Grok so that Grok speaks to you with its voice. You can also input your photos and ask Grok to edit them. You can also upload your documents and ask Grok to analyze them.

## [**Can I see my history of conversations with Grok?**](https://x.ai/legal/faq#can-i-see-my-history-of-conversations-with-grok)

Yes, you can view your past conversations with Grok on your app. On iOS, tap on your profile picture at the top left corner of your screen, and you will be brought to the conversation history page. On Grok.com, go to the top right corner of your screen and click on the history (icon with three horizontal lines and a magnifying glass).

![Image 1: History icon](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/history-icon-4cc80d7c.webp)History icon

Yes, you control what you choose to share. You can share your Grok conversations with others: (i) on [grok.com](https://grok.com/) by clicking "Share" in the top right corner of your browser or the share icon below your conversation to create and copy a public share link, or (ii) on the Grok Apps by tapping the share icon under your conversation to create a public share link. From there, you also control where you share your Grok conversation.

_**Note**: Any share link you generate will be accessible to anyone you choose to share the link with. For example, if you share the link publicly on a social media platform, it may be subject to indexing by a search engine (e.g., Google) just like any other publicly shared content._

Yes, you control what you share and how long it is shared. You can revoke access to any or all of your Grok conversation share links by logging into [grok.com](https://grok.com/), navigating to [grok.com/share-links](https://grok.com/share-links), and clicking "Remove" next to the share link you no longer wish to share.

## [**How can I make my conversations disappear?**](https://x.ai/legal/faq#how-can-i-make-my-conversations-disappear)

You can also choose not to have your conversations saved by using “Private Chat.” To activate Private Chat, look at the top right corner of your screen for the ghost shaped icon. When using Private Chat, your conversation history will not be viewable to you and will be deleted from SpaceXAI systems within 30 days.

## [**Who owns the inputs to and outputs from Grok?**](https://x.ai/legal/faq#who-owns-the-inputs-to-and-outputs-from-grok)

You own the Inputs and Outputs. You are free to use Grok’s Outputs (including generated images) from your conversations as you wish, including for commercial use. You own Grok’s Outputs and you grant SpaceXAI certain use rights pursuant to SpaceXAI’s [Consumer Terms](https://x.ai/legal/terms-of-service). We do ask that you attribute the generated work to Grok as provided in SpaceXAI’s Brand Guidelines available on [SpaceXAI’s legal website](https://x.ai/legal).

## [**How has Grok been pre-trained?**](https://x.ai/legal/faq#how-has-grok-been-pre-trained)

Grok has primarily been pre-trained on a large corpus of publicly available information, including raw web page data, metadata extracts, and text extracts from the Internet. This data helps Grok learn about associations between words and updates its model weights to support natural language processing. In other words, the training data helps Grok learn about language and how to understand queries and respond to them in ways that humans can comprehend. Importantly, this training refines the model's internal language structures. Once the training is done, the model does not reference or access the pre-training data.

Before using the training data, we apply quality filters and remove information that we do not want our models to learn from, such as violent content. We also apply security testing and evaluation measures. While Grok's training data may incidentally include personal information that is publicly available (for example, information about politicians, celebrities, or other public figures), SpaceXAI takes steps to minimize the processing of personal and sensitive data for training purposes. For clarity, we do not actively seek out personal information to train our models or use personal information to build individual profiles.

## [**Does Grok make mistakes?**](https://x.ai/legal/faq#does-grok-make-mistakes)

On occasion, Grok may provide inaccurate or inappropriate information. Because Grok has been trained on publicly available information, which may sometimes include misleading or factually inaccurate information, Grok may at times include in its responses misleading or factually incorrect information based upon that public information. Grok may also reflect positive or negative views of politicians, celebrities, and other public figures. Also, because artificial intelligence is rapidly evolving and is probabilistic in nature, Grok may therefore sometimes generate responses that contain “hallucinations,” or that otherwise may not be suitable for your intended purpose. You should carefully consider and verify Grok's responses before using them.

We constantly work to improve Grok. If you have feedback regarding a Grok response, we encourage you to send it to us.

## [**Do humans view my conversations?**](https://x.ai/legal/faq#do-humans-view-my-conversations)

SpaceXAI may use automated content classifiers and safety tools to better understand how our services are used and to ensure our Terms of Service and [Acceptable Use Policy](https://x.ai/legal/acceptable-use-policy) are not being violated.

A limited number of our authorized personnel may review your conversations with Grok for specific business purposes, including improving model performance, investigating security incidents and potential misuse of our services, and complying with our legal obligations. For more information, see our [Consumer Terms](https://x.ai/legal/terms-of-service) and [Privacy Policy](https://x.ai/privacy-policy).

## [**How can I use Grok responsibly?**](https://x.ai/legal/faq#how-can-i-use-grok-responsibly)

We believe that everyone (aged 13 and above) should be free to use our services in a way that suits their needs, so long as they comply with the law, don't harm themselves or others, and respect our guardrails. When using Grok, you must comply with our [Acceptable Use Policy](https://x.ai/legal/acceptable-use-policy) and the applicable Terms of Service for [Consumers](https://x.ai/legal/terms-of-service) or [Enterprise](https://x.ai/legal/terms-of-service-enterprise).

## [**How does SpaceXAI handle my personal information?**](https://x.ai/legal/faq#how-does-spacexai-handle-my-personal-information)

For more information about SpaceXAI's practices with respect to personal information, please see our [Privacy Policy](https://x.ai/privacy-policy).

## [**Does SpaceXAI use my content for model training?**](https://x.ai/legal/faq#does-spacexai-use-my-content-for-model-training)

We may use your content and interactions with Grok (e.g., prompts, searches, and other materials you submit) along with Grok's responses to train our models. Specifically, this information helps our models better understand human language and communication and therefore ultimately provide more accurate, relevant, and engaging responses. It also helps boost the capabilities and safety of our models. You control whether your data is used for training Grok. The section below, “How do I select whether my content is used for model training,” explains how to control it.

To protect your privacy, please do not share any personal or sensitive information in your questions to Grok.

When using Private Chat, where available, your content and interactions are not used for model training. Similarly, we do not use content from our business and enterprise customers to improve our models. For more details, please refer to our [Enterprise FAQ](https://x.ai/legal/enterprise/faq).

## [**How do I select whether my content is used for model training?**](https://x.ai/legal/faq#how-do-i-select-whether-my-content-is-used-for-model-training)

You can select whether or not your data is used for training in your Grok mobile app and the Grok.com website.

**Mobile App Data Controls for Training Grok:** On the mobile app, to select or deselect using your content for model training, go to Settings, Data Controls, and select or deselect “Improve the model.”

![Image 2: Mobile app data controls](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/app-data-6d629376.webp)Mobile app data controls

**Grok.com Data Controls for Training Grok:** For the Grok.com website, you can go to Settings, Data, and then “Improve the Model” to select whether your content is used for model training.

![Image 3: Web app data controls](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/web-data-15f61728.webp)Web app data controls

Once you select that your data should not be used for training, your new conversations will not be used to train our models. Alternatively, you can use Private Chat, and your content and interactions will not be included in model training.

Please note that, even if you opt out of model training, when you subsequently decide to voluntarily provide feedback, that feedback may be used for training purposes.

If you do not log into your account to access Grok (i.e., you are unauthenticated), where permissible, we may collect and retain your content on an anonymous basis. As a result, in some regions (excluding the EU/UK), when you use Grok without logging in, you won’t have the option to opt out of model training.

If you are accessing Grok in X, to opt-out of training for Grok, please see the [X Help Center](https://help.x.com/en/using-x/about-grok) for instructions.

## [**Can my Grok experience be personalized?**](https://x.ai/legal/faq#can-my-grok-experience-be-personalized)

Yes, your Grok experience can be personalized if you allow your X data and interactions to be used for personalization.

**Mobile App Data Controls for Personalization:** You can elect to allow your X data to be used for personalization on the Grok mobile app by going to Settings, Data Controls, and then toggle on or off “Personalize Grok using X”.

![Image 4: Mobile app data controls](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/app-data-6d629376.webp)Mobile app data controls

**Grok.com Data Controls for Personalization:** For the Grok.com website, you can go to Settings, Data, and then “Personalize Grok using X” to select whether you would like X data to be used for personalization on the Grok.com website.

![Image 5: Web app data controls](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/web-data-15f61728.webp)Web app data controls

If you are accessing Grok in X, please see the [X help pages](https://help.x.com/en/personalization-data-settings) on how to manage personalization settings.

## [**How to delete my data or account?**](https://x.ai/legal/faq#how-to-delete-my-data-or-account)

At any time, you can choose to delete selected conversations or all of your conversation history. Also, you can delete your account at any time.

**Mobile App Data & Account Deletion:** On your Grok mobile app, if you would like to delete a specific conversation, tap your profile picture to go to your conversation history, tap and hold down the name of the conversation you wish to delete, and then select “Delete Conversation.” On your Grok mobile app, to delete your Grok conversation history, go to Settings, Data Controls, and select “Delete All Conversations.” If you would like to delete your account, select “Delete Account.”

![Image 6: Mobile app data controls](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/app-data-6d629376.webp)Mobile app data controls

**Grok.com Data & Account Deletion:** On the grok.com website, if you want to delete a specific conversation, go to the conversation history page and select the trash button next to it. To delete all of your conversations or to delete your account, go to Settings and select the button next to what you would like to delete.

![Image 7: Web app data controls](https://media.x.ai/cdn-cgi/image/fit=scale-down,onerror=redirect,f=auto/v1/website/web-data-15f61728.webp)Web app data controls

If you are using Grok in X, to delete data or delete your X account, please see the [X Help Center](https://help.x.com/en/using-x/about-grok) for instructions.

After you indicate that you want your data deleted, it will take up to 30 days to delete from SpaceXAI systems.

## [**Does SpaceXAI sell my data?**](https://x.ai/legal/faq#does-spacexai-sell-my-data)

No. We do not sell your data or share it with third parties for marketing or advertising purposes. Thus, there is nothing you need to do to opt out of having your data sold or shared for marketing or advertising purposes.

## [**How long does SpaceXAI retain my information?**](https://x.ai/legal/faq#how-long-does-spacexai-retain-my-information)

You are in control of how long your data is on SpaceXAI’s systems. You can keep your data on your SpaceXAI account for as long as you wish. Users who do not register and log into Grok, will not have their data retained for their use after the session.

Also, you can delete your information when you wish. SpaceXAI retains your information for as long as reasonably necessary for the purposes described in our [Privacy Policy](https://x.ai/privacy-policy). If you delete conversations from your account or if you use Private Chat, conversations will be removed from our systems within 30 days, unless they have been de-identified or pseudoanonymized and disassociated from your account or we have to retain them for safety, security, or legal reasons.

## [**How do I submit a data subject rights request?**](https://x.ai/legal/faq#how-do-i-submit-a-data-subject-rights-request)

You are in control to access, download, and delete your data. Please go to your Grok mobile app or Grok.com (Settings/Data Controls) to delete or download your data. If you forget that you are in control of your data or if you have additional requests regarding your personal data, you can submit a request at [https://x.ai/privacy-portal/](https://x.ai/privacy-portal/). We will consider all requests that we receive and respond in accordance with applicable data protection laws. Please be aware that, in accordance with applicable laws, some rights are not absolute and we may decline your request if we have a lawful basis for doing so. If you feel we have not adequately addressed your request, you have the right to lodge a complaint with your local data protection authority.

For more information regarding your data protection and privacy rights, please see our [Privacy Policy](https://x.ai/legal/privacy-policy/).

If you have any questions or need assistance regarding the use of Grok or subscription issues, please don't hesitate to contact us at [support@x.ai](mailto:support@x.ai).
