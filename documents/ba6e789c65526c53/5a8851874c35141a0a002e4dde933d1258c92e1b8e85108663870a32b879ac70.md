Title: 选择地域、服务部署范围和接入域名

URL Source: https://help.aliyun.com/zh/model-studio/regions
...
[首页](https://help.aliyun.com/zh)[大模型服务平台百炼](https://help.aliyun.com/zh/model-studio/)[开始使用](https://help.aliyun.com/zh/model-studio/get-started-with-models/)选择地域、服务部署范围和接入域名

# 选择地域、服务部署范围和接入域名

更新时间：2026-09-17 16:39:29
...
![Image 3: image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9377607871/CAEQchiBgICkyp3I_hkiIGFjZmY5ZThjZWM2MzRkMmJhNTk5NWVmYTkwMWY3ZGI07466796_20260515102254.505.svg)
## 选择地域和服务部署范围
...
| **对比项** | **业务空间专属域名（推荐）** | **DashScope 域名（现有域名）** | **试用域名** |
| --- | --- | --- | --- |
| **域名格式** | `{WorkspaceId}.{region}.maas.aliyuncs.com` | `dashscope.aliyuncs.com` > 以华北 2（北京）地域为例 | `trial.{region}.maas.aliyuncs.com` |
| **适用场景** | 推荐在生产环境中使用，具备更高并发承载能力与网络隔离性，保障大流量场景下的稳定、低延迟访问体验。 | 存量业务兼容，建议[迁移至业务空间专属域名](https://help.aliyun.com/zh/model-studio/regions#h2_migrate_domain)。DashScope 域名（dashscope.aliyuncs.com）自 2026 年 9 月 30 日起不再支持新特性。 | 快速体验、功能验证，不建议用于生产环境。 |
| **鉴权方式** | 仅访问当前业务空间 | 可访问所有业务空间 | 可访问所有业务空间 |
| **限流额度** | RPM、TPM 按模型区分，主账号维度合并计算 | RPM、TPM 按模型区分，主账号维度合并计算 | RPM 为 1000（主账号维度），TPM 按模型区分 |
| **请求超时** | 3600 秒 | 600 秒 | 600 秒 |
| **协议支持** | HTTP、SSE、WebSocket、WebRTC、AOQ（AI over QUIC） | HTTP、SSE、WebSocket | HTTP、SSE |
| **SLA** | 99.9% | 99.9% | 不提供 |
...
**地域****地域 ID****业务空间专属域名****DashScope 域名****试用域名****API Key 入口****模型列表**

| **地域** | **地域 ID** | **业务空间专属域名** | **DashScope 域名** | **试用域名** | **API Key 入口** | **模型列表** |
| --- | --- | --- | --- | --- | --- | --- |
| 华北 2（北京） | `cn-beijing` | `{WorkspaceId}.cn-beijing.maas.aliyuncs.com` | `dashscope.aliyuncs.com` | `trial.cn-beijing.maas.aliyuncs.com` | [API Key](https://bailian.console.aliyun.com/cn-beijing/model/settings/api-key) | [可用模型](https://bailian.console.aliyun.com/cn-beijing/model/market) |
| 新加坡 | `ap-southeast-1` | `{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com` | `dashscope-intl.aliyuncs.com` | `trial.ap-southeast-1.maas.aliyuncs.com` | [API Key](https://bailian.console.aliyun.com/ap-southeast-1/model/settings/api-key) | [可用模型](https://bailian.console.aliyun.com/ap-southeast-1/model/market) |
| 德国（法兰克福） | `eu-central-1` | `{WorkspaceId}.eu-central-1.maas.aliyuncs.com` | 不支持 | 暂不支持 | [API Key](https://bailian.console.aliyun.com/eu-central-1/model/settings/api-key) | [可用模型](https://bailian.console.aliyun.com/eu-central-1/model/market) |
| 日本（东京） | `ap-northeast-1` | `{WorkspaceId}.ap-northeast-1.maas.aliyuncs.com` | 不支持 | 暂不支持 | [API Key](https://bailian.console.aliyun.com/ap-northeast-1/model/settings/api-key) | [可用模型](https://bailian.console.aliyun.com/ap-northeast-1/model/market) |
| 中国香港 | `cn-hongkong` | `{WorkspaceId}.cn-hongkong.maas.aliyuncs.com` | `cn-hongkong.dashscope.aliyuncs.com` | `trial.cn-hongkong.maas.aliyuncs.com` | [API Key](https://bailian.console.aliyun.com/cn-hongkong/model/settings/api-key) | [可用模型](https://bailian.console.aliyun.com/cn-hongkong/model/market) |
| 美国（弗吉尼亚） | `us-east-1` | `{WorkspaceId}.us-east-1.maas.aliyuncs.com` | `dashscope-us.aliyuncs.com` | 暂不支持 | [API Key](https://bailian.console.aliyun.com/us-east-1/model/settings/api-key) | [可用模型](https://bailian.console.aliyun.com/us-east-1/model/market) |
...
*   **华北 2（北京）**和**新加坡地域**各仅支持一种服务部署范围，无需选择。

**说明**

美国（弗吉尼亚）地域的部署模式已升级。如果正在使用带 `-us` 后缀的模型，请尽快完成切换升级，详情请参见[升级公告](https://www.aliyun.com/notice/118646)。
...
1.   **获取业务空间专属域名**：
    ...
    *   方式一：在[API Key 创建](https://bailian.console.aliyun.com/cn-beijing/model/settings/api-key)后的弹窗中，复制 **API Host** 。
    *   方式二：在[业务空间管理](https://bailian.console.aliyun.com/cn-beijing/settings/workspace)页面，复制 **API Host** 列的内容。

2.   **替换 Base URL 中的域名**：将原域名替换为业务空间专属域名。以华北 2（北京）地域为例，`llm-xxx` 为业务空间 ID：
...
**功能****华北 2（北京）****新加坡****美国（弗吉尼亚）****中国香港****德国（法兰克福）****日本（东京）**

| **功能** | **华北 2（北京）** | **新加坡** | **美国（弗吉尼亚）** | **中国香港** | **德国（法兰克福）** | **日本（东京）** |
| --- | --- | --- | --- | --- | --- | --- |
| 实时推理 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| 批量推理 | 支持 | 支持 | 不支持 | 不支持 | 不支持 | 不支持 |
| 模型体验 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| 模型监控（普通） | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| 模型监控（高级） | 支持 | 支持 | 支持 | 不支持 | 不支持 | 不支持 |
| 模型告警 | 支持 | 支持 | 不支持 | 不支持 | 不支持 | 不支持 |
| 传输安全 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| 权限管理 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| 模型调优 | 支持 | 不支持 | 不支持 | 不支持 | 不支持 | 不支持 |
| 应用开发 | 支持 | 不支持 | 不支持 | 不支持 | 不支持 | 不支持 |
...
*   [Base URL 总览](https://help.aliyun.com/zh/model-studio/base-url) — 模型服务调用地址
*   [概述](https://help.aliyun.com/zh/model-studio/realtime-api-overview)

[上一篇：Base URL总览](https://help.aliyun.com/zh/model-studio/base-url)[下一篇：新人免费额度](https://help.aliyun.com/zh/model-studio/new-free-quota)
...
![Image 4: 阿里云APP](https://img.alicdn.com/imgextra/i4/O1CN01XLesV31fkf7pYNATb_!!6000000004045-2-tps-400-400.png)![Image 5: 阿里云微信](https://img.alicdn.com/tfs/TB1AOdINW6qK1RjSZFmXXX0PFXa-258-258.jpg)
联系我们：4008013260
...
##### ![Image 8: 文本生成](https://img.alicdn.com/imgextra/i1/O1CN01cG8ZE02192EvBEBSH_!!6000000006941-55-tps-24-24.svg)文本生成

###### [![Image 9: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Qwen3.8-Max _HOT_ 0902 快照版本重磅更新，智能体时代全能旗舰模型](https://platform.qianwenai.com/try-ai?models=qwen3.8-max-0902)###### [![Image 10: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Qwen3.8-Flash _NEW_ 百万级上下文窗口，一次性处理超长文档和代码仓库](https://platform.qianwenai.com/try-ai?models=qwen3.8-flash)###### [![Image 11: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Qwen3-VL-Plus 视觉 Coding、空间感知、多模态思考等全面升级](https://platform.qianwenai.com/try-ai?models=qwen3-vl-plus)

###### [![Image 12: Kimi-K3](https://img.alicdn.com/imgextra/i4/O1CN01zZ5N8O1FvUau58xvk_!!6000000000549-55-tps-14-14.svg)Kimi-K3 Kimi 最新旗舰模型，长程编程与推理利器](https://www.qianwenai.com/models/kimi%2Fkimi-k3)###### [![Image 13: Deepseek-v4-pro](https://img.alicdn.com/imgextra/i3/O1CN01gcs2521tgSDH7j5NC_!!6000000005931-55-tps-14-14.svg)Deepseek-v4-pro 旗舰 MoE 大模型，百万上下文与顶尖推理能力](https://platform.qianwenai.com/try-ai?models=deepseek-v4-pro)###### [![Image 14: GLM-5.2](https://img.alicdn.com/imgextra/i1/O1CN01vxXuZt1K85wuj07CP_!!6000000001118-55-tps-14-14.svg)GLM-5.2 1M上下文，专为长程任务能力而生](https://platform.qianwenai.com/try-ai?models=glm-5.2)

##### ![Image 15: 图片和视频生成](https://img.alicdn.com/imgextra/i4/O1CN011o40eW20rdBmYijum_!!6000000006903-55-tps-24-24.svg)图片和视频生成

###### [![Image 16: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Wan3.0-Video 四模态全能参考，多种创作一站支持](https://platform.qianwenai.com/try-ai?scene=video&models=wan3.0-video)###### [![Image 17: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Qwen-Image-3.0-Pro _NEW_ 细节真实，知识厚实，让图像生成真正成为可落地的生产力工具](https://platform.qianwenai.com/try-ai/chat?models=qwen-image-3.0-pro)###### [![Image 18: HappyHorse-1.1-T2V](https://img.alicdn.com/imgextra/i1/O1CN01kQsuEF1xljEn4gAbh_!!6000000006484-55-tps-14-14.svg)HappyHorse-1.1-T2V 让文字生成流畅细腻的高质量视频](https://platform.qianwenai.com/try-ai?models=happyhorse-1.1-t2v)

##### ![Image 19: 语音识别与合成](https://img.alicdn.com/imgextra/i2/O1CN01UBMw1L1d3x46Vr1q3_!!6000000003681-55-tps-24-24.svg)语音识别与合成

###### [![Image 20: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Qwen3-TTS-Flash 离线语音合成大模型，多语言方言自适应，低延迟高稳定](https://platform.qianwenai.com/try-ai?scene=tts&models=qwen3-tts-flash)###### [![Image 21: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Cosyvoice-V3-Flash 高表现力语音合成大模型，语音克隆听感自然](https://platform.qianwenai.com/try-ai?scene=tts&models=cosyvoice-v3-flash)###### [![Image 22: Fun-ASR](https://img.alicdn.com/imgextra/i4/O1CN01aF2Ya21gXfc0oMugD_!!6000000004152-55-tps-14-14.svg)Fun-ASR 支持中英文自由切换，具备更强的噪声鲁棒性](https://platform.qianwenai.com/try-ai?models=fun-asr)
...
[![Image 23: 阿里云 OPC 创新助力计划](https://img.alicdn.com/imgextra/i3/O1CN01vBbQLH1XUcOADg2M5_!!6000000002927-2-tps-460-544.png) ##### 阿里云 OPC 创新助力计划 至高可申请百万元 Token 补贴，五大权益加速 OPC 成功](https://opc.aliyun.com/)
...
4008013260[售前咨询](https://smartservice.console.aliyun.com/pre-sale/chat?entrance=201&referrer=https%3A%2F%2Fhelp.aliyun.com%2Fzh%2Fmodel-studio%2Fregions)[售后在线](https://smartservice.console.aliyun.com/service/robot-chat?entrance=201&referrer=https%3A%2F%2Fhelp.aliyun.com%2Fzh%2Fmodel-studio%2Fregions)
### 其他服务

[我要建议](https://www.aliyun.com/connect/home)[我要投诉](https://www.aliyun.com/complaint)

![Image 24: 登录插画](https://img.alicdn.com/imgextra/i2/O1CN015QIT9m1FmmyUntYlQ_!!6000000000530-2-tps-320-200.png)
登录以查看您的控制台资源
...
[快捷注册](https://account.aliyun.com/register/qr_register.htm?oauth_callback=https%3A%2F%2Fhelp.aliyun.com%2Fzh%2Fmodel-studio%2Fregions)[登录阿里云](https://account.aliyun.com/login/login.htm?oauth_callback=https%3A%2F%2Fhelp.aliyun.com%2Fzh%2Fmodel-studio%2Fregions)
