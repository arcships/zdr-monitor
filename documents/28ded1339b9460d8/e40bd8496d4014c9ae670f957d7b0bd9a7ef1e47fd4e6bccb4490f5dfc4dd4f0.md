Title: region inference | Snowflake Documentation

URL Source: https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference
...
Snowflake Cortex AI supports cross-region inference across AWS, Azure, and GCP regions. You control the routing behavior through a single account-level parameter, [CORTEX_ENABLED_CROSS_REGION](https://docs.snowflake.com/sql-reference/parameters#label-cortex-enable-cross-region), choosing the option that best fits your performance, compliance, and data residency requirements.

## How it works[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#how-it-works)
...
## Choosing a routing option[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#choosing-a-routing-option)
...
### Any region[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#any-region)
...
For new accounts created in new organizations within commercial regions after March 9, 2026, `ANY_REGION` is the default. For other accounts, see [Default routing](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#label-cortex-cross-region-default).

### Specify cloud regions[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#specify-cloud-regions)
...
For a full list of supported regions, see the [CORTEX_ENABLED_CROSS_REGION](https://docs.snowflake.com/sql-reference/parameters#label-cortex-enable-cross-region) parameter reference.

### Account region only[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#account-region-only)
...
[](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference)
### Default routing[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#default-routing)

If you have not set this parameter, your account inherits a default. New organizations created after March 9, 2026 use `ANY_REGION`. Other commercial accounts in supported regions inherit a same-cloud geography value such as `AWS_US` or `AZURE_EU`. For the default that applies to your region, see [CORTEX_ENABLED_CROSS_REGION](https://docs.snowflake.com/sql-reference/parameters#label-cortex-enable-cross-region) and the [2026_06 behavior change](https://docs.snowflake.com/release-notes/bcr-bundles/2026_06/bcr-2376).

SHOW PARAMETERS LIKE 'CORTEX_ENABLED_CROSS_REGION' IN ACCOUNT;

The `value` column is the value in effect. If you have data residency requirements, set the parameter explicitly.

## Data security and residency[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#data-security-and-residency)
...
## US Commercial Gov regions[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#us-commercial-gov-regions)
...
## US FedRAMP High and DoD gov regions[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#us-fedramp-high-and-dod-gov-regions)
...
[](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference)
## Access control requirements[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#access-control-requirements)

The `CORTEX_ENABLED_CROSS_REGION` parameter can only be set at the account level, not at the user or session levels. Only the ACCOUNTADMIN role can modify this parameter using the [ALTER ACCOUNT](https://docs.snowflake.com/sql-reference/sql/alter-account) command. This parameter cannot be set by the ORGADMIN role.

ALTER ACCOUNT SET CORTEX_ENABLED_CROSS_REGION = 'ANY_REGION';

[](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference)
## Cost considerations[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#cost-considerations)
...
## Additional considerations[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#additional-considerations)
...
*   Cross-region inference for [Cortex Search](https://docs.snowflake.com/user-guide/snowflake-cortex/cortex-search/cortex-search-overview) is not supported in [all regions](https://docs.snowflake.com/user-guide/snowflake-cortex/cortex-search/cortex-search-overview#label-cortex-search-overview-regional-availability).

## Next steps[¶](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cross-region-inference#next-steps)

*   For details on the cross-region inference parameter, see the [CORTEX_ENABLED_CROSS_REGION](https://docs.snowflake.com/sql-reference/parameters#label-cortex-enable-cross-region) section of the SQL parameter reference.
